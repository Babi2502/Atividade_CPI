...
Vin(s) = (R1+R2+Rp)*I(s)+I(s)/sC1+I(s)/sC2

A função trasferência G(s) = I(s)/Vin(s)
G(s) = 1/(R1+R2+Rp)s+(1/C1 + 1/C2)

Pelo metodo de Skogestad
Kp = 1/K*(tau + theta)
Ki = Kp/tau
Kd = Kp*tau/2
*/
...
double K = 1.0;  // Ganho estático do sistema 
double tau = 0.5;  // Constante de tempo do sistema
...
void loop()
{
  ...
  Serial.print(",");
  Serial.print(real);                // Imprime o valor atual do sensor
  Serial.print(",");
